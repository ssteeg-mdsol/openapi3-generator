...
const removeRemainingRefs = (obj) => {
  Object.keys(obj).forEach(key => {
    if (key === '$ref') {
      let value = obj[key].split("\/").slice(-1)[0];
      delete(obj[key]);
      obj["type"] = value;
    } else if (obj[key] && typeof obj[key] === 'object') {
      removeRemainingRefs(obj[key])
    }
  });
  return obj;
};
...
const beautifySchema = (schema) => {
  ...
  if (!schema.example) {
    // Workaround for handling circular refs. Remaining refs are replaced with { "type": "SchemaName" }
    removeRemainingRefs(schema);
    schema.generatedExample = generateExample(schema);
  }
